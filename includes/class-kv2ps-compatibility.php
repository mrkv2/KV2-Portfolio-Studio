...
final class KV2PS_Compatibility {
	...
	/**
	 * Convert native shortcode taxonomy attributes into an AND tax query.
	 *
	 * Each attribute accepts one slug or a comma/pipe-separated list. Multiple
	 * values inside the same attribute use WordPress' default IN semantics,
	 * while clauses from different taxonomies remain combined with AND.
	 */
	public static function shortcode_tax_query( $atts, $query = array() ) {
		$atts  = is_array( $atts ) ? $atts : array();
		...
		foreach ( $map as $attribute => $taxonomy ) {
			...
			$values = preg_split( '/[,|]+/', (string) $atts[ $attribute ] );
			$terms  = array_map(
				function( $value ) use ( $taxonomy ) {
					return 'kv2_ville' === $taxonomy ? KV2PS_Post_Types::city_slug( $value ) : sanitize_title( $value );
				},
				(array) $values
			);
