...
while ( have_posts() ) :
	...
	<main id="primary" class="kv2ps-main kv2ps-single">
		...
		if ( $related->have_posts() ) :
			?>
			<section class="kv2ps-related" aria-labelledby="kv2ps-related-title"><div class="kv2ps-container"><div class="kv2ps-related__header"><div><span class="kv2ps-eyebrow"><?php esc_html_e( 'À découvrir ensuite', 'kv2-portfolio-studio' ); ?></span><h2 id="kv2ps-related-title"><?php esc_html_e( 'D’autres réalisations', 'kv2-portfolio-studio' ); ?></h2><?php if ( $service_terms && ! is_wp_error( $service_terms ) ) : ?><p><?php esc_html_e( 'Une sélection dans les mêmes catégories que ce projet.', 'kv2-portfolio-studio' ); ?></p><?php endif; ?></div><?php $portfolio_url = ! empty( $settings['portfolio_page_url'] ) ? $settings['portfolio_page_url'] : get_post_type_archive_link( KV2PS_Post_Types::POST_TYPE ); if ( $portfolio_url ) : ?><a class="kv2ps-related__all" href="<?php echo esc_url( $portfolio_url ); ?>"><?php esc_html_e( 'Toutes les réalisations', 'kv2-portfolio-studio' ); ?> <span aria-hidden="true">→</span></a><?php endif; ?></div><div class="kv2ps-grid kv2ps-layout-masonry kv2ps-cols-3 kv2ps-ratio-auto kv2ps-card-style-classic">
